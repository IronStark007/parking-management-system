...
const createBooking = (req, res) => {
  ...
  if (!parked) {
    ...
    return res
      .status(500)
      .send({ error: `parking slot with id ${id} not exists` });
  }
  const booked = booking.filter(
    (booking) => booking.bookingId == parked.bookingId
  )[0];
  if (booked.booking) {
    console.error("Slot is occupied");
    return res.status(500).send({ message: "Slot is occupied" });
  }
  const payed = payment.filter(
    (payment) => payment.bookingId == booked.bookingId
  )[0];
  if (!payed.payment) {
    console.log("Slot is free, please do the payment");
    return res
      .status(200)
      .send({ message: "Slot is free, please do the payment" });
  } else {
    booked.booking = true;
...
const deleteBooking = (req, res) => {
  ...
  const booked = booking.filter(
    (booking) => booking.slotId == parked.slotId
  )[0];
  const payed = payment.filter(
    (booking) => booking.bookingId == booked.bookingId
  )[0];
  if (booked.booking) {
